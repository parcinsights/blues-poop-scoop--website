...
export const redirects = [
  ...
  //
  // NO /residential RULE. There was one — the old site's recurring-service page was pointed first
  // at /services/weekly-scooping/ and then, when the frequency pages were merged away, at
  // /services/poop-scoop/. It was removed on 2026-08-03 because /residential/ is now a real page of
  // ours, and a redirect on that source swallows it: the nav link, the sitemap entry and every
  // inbound link to the old URL all landed on the service page instead. The old URL's closest
  // equivalent is the page that now answers on it, which is the best outcome the 301 was ever
  // reaching for. See app/residential/page.tsx.
  { source: "/commercial-services", destination: "/commercial/", permanent: true },

  // Ours, not WordPress's. /get-started/ shipped as a second quote page and was merged into
  // /contact/ on 2026-08-02 — one form, one destination for every CTA. It was in the sitemap, so
  // it may already be indexed; a 301 is what hands those signals to /contact/. See lib/routes.ts.
